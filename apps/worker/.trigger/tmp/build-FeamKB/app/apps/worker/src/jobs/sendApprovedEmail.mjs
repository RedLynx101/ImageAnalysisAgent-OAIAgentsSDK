...
var require_shared = __commonJS({
  "../../node_modules/nodemailer/lib/shared/index.js"(exports, module) {
    ...
    module.exports._logFunc = (logger3, level, defaults, data, message, ...args) => {
      let entry = {};
...
var createTransporter = async () => {
  if (!import_shared.env.GMAIL_APP_PASSWORD) {
    throw new Error("GMAIL_APP_PASSWORD is not set in environment variables.");
  }
  const transporter = import_nodemailer.default.createTransport({
    service: "gmail",
    auth: {
      user: import_shared.env.GMAIL_SENDER_EMAIL,
      pass: import_shared.env.GMAIL_APP_PASSWORD
    }
  });
  return transporter;
};
var sendEmail = async ({ recipient, pdfPath }) => {
  import_shared.logger.info(`Sending email to ${recipient || import_shared.env.GMAIL_SENDER_EMAIL} with attachment ${pdfPath}`);
  try {
    const transporter = await createTransporter();
    const mailOptions = {
      from: import_shared.env.GMAIL_SENDER_EMAIL,
      to: recipient || import_shared.env.GMAIL_SENDER_EMAIL,
    ...
    import_shared.logger.info("Email sent successfully");
  } catch (error) {
    import_shared.logger.error("Error sending email:", error);
    throw error;
  }
};

// src/jobs/sendApprovedEmail.ts
var sendApprovedEmail = task({
...
//# sourceMappingURL=sendApprovedEmail.mjs.map
